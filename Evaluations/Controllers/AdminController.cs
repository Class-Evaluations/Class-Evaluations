    ...
    public class AdminController : Controller
    {
         ...
         public ActionResult LoadMongoDB()
        {
            ...
            //get the survey sent table to import the survey information
             var surveyData = (from s in _survey.COURSE_STATUS select s).ToList();
            ...
            foreach (var item in courseList)
            {
                ...
                foreach (var surveyItem in surveyData)
                {
                    //try(
                    if (surveyItem.course_id == item.course_id)
                    {
                        CourseDoc.Add("SurveyStatus", surveyItem.course_status1);
                        CourseDoc.Add("ExpirationDate", surveyItem.survey_exp_date);
                    }
                   //catch
                   //{
